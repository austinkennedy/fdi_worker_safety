...
wa_mod1 <- feols(tcr ~ log(fdi_tot) + log(fdi_tot)*weight_avg, odi_plant, cluster = c("Year", "naics_3"))
wa_mod2 <- feols(tcr ~ log(fdi_tot) + log(fdi_tot)*weight_avg|Year, odi_plant, cluster = c("Year", "naics_3"))
wa_mod3 <- feols(tcr ~ log(fdi_tot) + log(fdi_tot)*weight_avg|Year + naics_3, odi_plant, cluster = c("Year", "naics_3"))
wa_mod4 <- feols(tcr ~ log(fdi_tot) + log(fdi_tot)*weight_avg|naics_3, odi_plant, cluster = c("Year", "naics_3"))
...
cm <- c("(Intercept)" = "Intercept", "davg" = "Safety", "log(fdi)" = "$ln(FDI)$")
...
kableExtra::save_kable(tab_d, file = "../Output/table_d.tex")

```

```{r Deviation Interaction table}

cm <- c("(Intercept)" = "Intercept", "davg" = "Safety", "log(fdi)" = "$ln(FDI)$", "log(fdi_tot):davg" = "$ln(FDI) \\times Safety$")
...
tab_dint <- modelsummary(dint_models,
             stars = TRUE,
             coef_map = cm,
             title = cap,
             gof_omit = "R2 Within|R2 P|Log|AIC|BIC|Std",
...
kableExtra::save_kable(tab_dint, file = "../Output/table_dint.tex")
