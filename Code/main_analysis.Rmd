...
basic_shares <- ind_year %>%
  feols(tcr~safe_share*log(fdi), cluster = c('NAICS'))

basic_shares_time <- ind_year %>%
  feols(tcr~safe_share*log(fdi)|Year, cluster = c('NAICS'))

basic_shares_twoway <- ind_year %>%
  feols(tcr~safe_share + log(fdi)|Year + NAICS, cluster = c('NAICS'))
...
agg_ind_shares_twoway <- ind_year %>%
  feols(tcr ~ safe_industry*safe_share|Year+NAICS, cluster = c('NAICS'))

agg_ind_shares_fdi <- ind_year %>%
  feols(tcr ~ safe_industry*safe_share + log(fdi), cluster = c('NAICS'))

agg_ind_shares_year_fdi <- ind_year %>%
  feols(tcr ~ safe_industry*safe_share + log(fdi)|Year, cluster = c('NAICS'))

agg_ind_shares_twoway_fdi <- ind_year %>%
  feols(tcr ~ safe_industry*safe_share + log(fdi)*safe_share|Year+NAICS, cluster = c('NAICS'))
...
agg_state_shares_twoway <- state_ind_year %>%
  feols(tcr ~ safe_state*safe_share|Year+NAICS, cluster = c('NAICS'))

agg_state_shares_fdi <- state_ind_year %>%
  feols(tcr ~ safe_state*safe_share + log(fdi), cluster = c('NAICS'))

agg_state_shares_year_fdi <- state_ind_year %>%
  feols(tcr ~ safe_state*safe_share + log(fdi)|Year, cluster = c('NAICS', 'STATE'))

agg_state_shares_twoway_fdi <- state_ind_year %>%
  feols(tcr ~ safe_state*safe_share + log(fdi)*safe_share|Year+NAICS+STATE, cluster = c('NAICS', 'STATE'))

agg_ind_state_share_twoway_fdi<- state_ind_year %>%
  feols(tcr ~ safe_state*safe_industry*safe_share + log(fdi)*safe_share|Year+NAICS+STATE, cluster = c('NAICS', 'STATE'))
...
```{r Full shares table}
shares_models <- list(basic_shares_twoway, basic_shares_twoway_interaction, agg_ind_shares_twoway_fdi, agg_state_shares_twoway_fdi, agg_ind_state_share_twoway_fdi)
...
agg_state_shares_tab <- modelsummary(shares_models,
                               ...
                               escape = FALSE,
                               fmt = "%.4f") %>%
  add_header_above(c(" " = 1, "Industry-Level" = 3, "State-Level" = 2)) %>%
  add_footnote(fn, threeparttable = TRUE)
