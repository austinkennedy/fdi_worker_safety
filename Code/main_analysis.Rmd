...
basic_shares_twoway <- ind_year %>%
  feols(tcr~safe_share|Year + NAICS, cluster = c('NAICS'))

basic_shares_twoway_fdi <- ind_year %>%
...
agg_ind_shares_twoway_fdi <- ind_year %>%
  feols(tcr ~ safe_industry*safe_share + log(fdi)|Year+NAICS, cluster = c('NAICS'))
...
agg_state_shares_year_fdi <- state_ind_year %>%
  feols(tcr ~ safe_state*safe_share + log(fdi)|Year, cluster = c('NAICS'))

agg_state_shares_twoway_fdi <- state_ind_year %>%
  feols(tcr ~ safe_state*safe_share + log(fdi)|Year+NAICS, cluster = c('NAICS'))
...
```{r Full shares table}
shares_models <- list(basic_shares_twoway, basic_shares_twoway_fdi, agg_ind_shares_twoway, agg_ind_shares_twoway_fdi, agg_state_shares_twoway, agg_state_shares_twoway_fdi)

fn <- "Standard errors clustered at the industry level columns (1) through (4), clustered at the industry-state level for columns (5) and (6)."
