...
agg_ind <- ind_year %>%
  feols(tcr ~ safe*log(fdi), cluster = c('NAICS'))

agg_ind_year <- ind_year %>%
  feols(tcr ~ safe*log(fdi)|Year, cluster = c('NAICS'))

agg_ind_twoway <- ind_year %>%
  feols(tcr ~ safe*log(fdi)|Year+NAICS, cluster = c('NAICS'))
...
agg_state_year <- state_ind_year %>%
  feols(tcr ~ safe*log(fdi)|Year, cluster = c('STATE','NAICS'))

agg_state_twoway <- state_ind_year %>%
  feols(tcr ~ safe*log(fdi)|Year+NAICS, cluster = c('STATE','NAICS'))

agg_state_threeway <- state_ind_year %>%
  feols(tcr ~ safe*log(fdi)|Year+NAICS+STATE, cluster = c('STATE','NAICS'))
...
basic_shares_regs <- list(basic_shares, basic_shares_time, basic_shares_twoway)
...
agg_ind_shares <- ind_year %>%
  feols(tcr ~ safe*safe_share, cluster = c('NAICS'))

agg_ind_shares_year <- ind_year %>%
  feols(tcr ~ safe*safe_share|Year, cluster = c('NAICS'))

agg_ind_shares_twoway <- ind_year %>%
  feols(tcr ~ safe*safe_share|Year+NAICS, cluster = c('NAICS'))

agg_ind_shares_fdi <- ind_year %>%
  feols(tcr ~ safe*safe_share + log(fdi), cluster = c('NAICS'))

agg_ind_shares_year_fdi <- ind_year %>%
  feols(tcr ~ safe*safe_share + log(fdi)|Year, cluster = c('NAICS'))

agg_ind_shares_twoway_fdi <- ind_year %>%
  feols(tcr ~ safe*safe_share + log(fdi)|Year+NAICS, cluster = c('NAICS'))
...
agg_state_shares <- ind_year %>%
  feols(tcr ~ safe*safe_share, cluster = c('NAICS'))

agg_state_shares_year <- ind_year %>%
  feols(tcr ~ safe*safe_share|Year, cluster = c('NAICS'))

agg_state_shares_twoway <- ind_year %>%
  feols(tcr ~ safe*safe_share|Year+NAICS, cluster = c('NAICS'))

agg_state_shares_fdi <- ind_year %>%
  feols(tcr ~ safe*safe_share + log(fdi), cluster = c('NAICS'))

agg_state_shares_year_fdi <- ind_year %>%
  feols(tcr ~ safe*safe_share + log(fdi)|Year, cluster = c('NAICS'))

agg_state_shares_twoway_fdi <- ind_year %>%
  feols(tcr ~ safe*safe_share + log(fdi)|Year+NAICS, cluster = c('NAICS'))
...
cm <- c("(Intercept)" = "Intercept",
        "log(fdi)" = "$ln(FDI)$",
        "safe" = "$Safe$",
        "safe:log(fdi)" = "$Safe \\times ln(FDI)$",
        "safe_share" = "$Share^{Safe FDI}$",
        "safe:safe_share" = "$Safe \\times Share^{Safe FDI}$")
...
tab_basic_regs <- modelsummary(basic_regs,
                               stars = star_map,
                               coef_map = cm,
...
cm <- c("(Intercept)" = "Intercept",
        "log(fdi)" = "$ln(FDI)$",
        "safe" = "$SafeIndustry$",
        "safe:log(fdi)" = "$SafeIndustry \\times ln(FDI)$")
...
cm <- c("(Intercept)" = "Intercept",
        "log(fdi)" = "$ln(FDI)$",
        "safe" = "$SafeState$",
        "safe:log(fdi)" = "$SafeState \\times ln(FDI)$")
