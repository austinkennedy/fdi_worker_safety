...
# years <- as.numeric(fdi_ind[2,])
# industries <- colnames(fdi_ind)
# 
# #Function to insert row
# insertRow <- function(existingDF, newrow, r) {
#   existingDF[seq(r+1,nrow(existingDF)+1),] <- existingDF[seq(r,nrow(existingDF)),]
...
# }
# 
# fdi_ind <- insertRow(fdi_ind, industries, 1) #Insert colnames as a row to not be lost
# colnames(fdi_ind) <- years #Change col names to years for wide to long transition
# 
# fdi_ind <- t(fdi_ind) #transpose
# #Change col names to countries
# countries <- fdi_ind[1,] 
# colnames(fdi_ind) <- countries
# fdi_ind <- fdi_ind[-1,]
# 
# colnames(fdi_ind)[1:3] = c("title_2", "title_3", "Year")
# rownames(fdi_ind) <- NULL
# fdi_ind <- as_tibble(fdi_ind)
# fdi <- fdi %>% pivot_longer(-c("title_2", "title_3", "Year"), names_to = "Country", values_to = "fdi") #Wide to long
```
